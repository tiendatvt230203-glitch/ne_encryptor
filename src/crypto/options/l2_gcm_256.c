...
static int l2_do_decrypt(struct packet_crypto_ctx *ctx, uint8_t *packet, size_t pkt_len)
{
    ...
    if (unlikely(crypto_aes_gcm_decrypt(key, nonce, L2_NONCE_SIZE, work_ptr, (int)enc_len, tag, OPT_AES_BITS) != 0))
        return -1;
    ...
    uint16_t *out_pkt_id, uint8_t *out_frag_index)
{
    ...
    if (nonce_off < 0)
        return -1;
    if (pkt_len - (size_t)enc_off < AES_GCM_TAG_SIZE)
        return -1;
    enc_len = pkt_len - (size_t)enc_off - AES_GCM_TAG_SIZE;
    ...
    for (int k = 0; k < KEY_SLOT_COUNT; k++) {
        ...
        memcpy(nonce, packet + nonce_off, (size_t)L2_NONCE_SIZE);
        if (crypto_aes_gcm_decrypt(key, nonce, L2_NONCE_SIZE, work, (int)enc_len, tag, OPT_AES_BITS) != 0)
            continue;
...
int crypto_opt_l2_gcm256_need_split(uint32_t pkt_len)
{
    return (pkt_len + OPT_FRAG_META_LEN) > crypto_option_get_mtu();
}

int crypto_opt_l2_gcm256_split(struct packet_crypto_ctx *ctx, uint8_t *pkt_data, uint32_t pkt_len,
                           size_t frag0_max, uint32_t *frag0_len,
                           uint8_t *frag1, size_t frag1_max, uint32_t *frag1_len)
{
...
int crypto_opt_l2_gcm256_encrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int l3_off;
    int et_off;
    int n;
...
int crypto_opt_l2_gcm256_decrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int n;
...
int crypto_opt_l2_gcm256_is_fragment(const struct app_config *cfg, const uint8_t *pkt_data,
                                 uint32_t pkt_len, uint16_t *pkt_id, uint8_t *frag_index)
{
    ...
    if (crypto_eth_l2_read_policy_id(pkt_data, pkt_len, &wire_pol) != 0)
        return 0;
    if (!opt_policy_match(cfg, POLICY_ACTION_ENCRYPT_L2, CRYPTO_MODE_GCM, 256, wire_pol))
        return 0;
    opt_read_frag_tag(pkt_data + tag_off + 1, pkt_id, frag_index);
    return (*frag_index <= 1) ? 1 : 0;
}

int crypto_opt_l2_gcm256_reasm(int profile_slot, int worker_idx, struct packet_crypto_ctx *ctx,
                           ...
                           uint8_t *out_buf, uint32_t *out_len)
{
    ...
    if (nd < 0)
        return -1;
    *pkt_len = (uint32_t)nd;
    rr = l2_reassemble(opt_table(profile_slot, worker_idx), pkt_data, *pkt_len,
...
void crypto_opt_l2_gcm256_frag_gc(int profile_slot, int worker_idx, uint64_t now_ns)
{
