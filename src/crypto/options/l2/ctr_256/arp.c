...
/*
 * L2 ARP overhead only (no cipher yet).
 *   [dst|src|0x88B6|policy_id|core_id|nonce|ARP body]
...
static int l2_do_detach_arp(struct packet_crypto_ctx *ctx, uint8_t *packet, size_t pkt_len)
{
    ...
    if (unlikely(!l2_verify_arp_body(work_ptr, payload_len)))
        return -1;
    return l2_restore_plain_arp(packet, pkt_len, work_ptr, payload_len);
}
...
CRYPTO_OPS_PLAIN(crypto_opt_l2_ctr256_arp_ops, arp_encrypt, arp_decrypt)
