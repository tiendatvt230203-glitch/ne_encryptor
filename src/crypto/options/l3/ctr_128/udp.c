    ...
    uint8_t *out_buf, size_t out_max, uint32_t *out_len, uint8_t fake_proto)
{
    ...
    if (crypto_aes_ctr_with_key(key, iv, out_buf + enc_off, (int)enc_plain_len, OPT_AES_BITS) != 0)
        return -1;
    ip_payload_len = (size_t)(L3_FRAG_TUNNEL_HDR_SIZE + L3_FRAG_TAG_SIZE + enc_plain_len);
    ...
    uint16_t pkt_id, size_t out_max, uint32_t *out_len)
{
    ...
    if (crypto_aes_ctr_with_key(key, iv, packet + enc_off, (int)frag0_plain_len, OPT_AES_BITS) != 0)
        return -1;
    l3_patch_ipv4_fast(ip, (uint16_t)(ip_hdr_len + L3_FRAG_TUNNEL_HDR_SIZE + L3_FRAG_TAG_SIZE + frag0_plain_len), fake_proto);
    *out_len = (uint32_t)need;
    return 0;
}
                                 ...
                                 uint32_t pkt_len, uint16_t *pkt_id, uint8_t *frag_index)
{
    ...
    if (!opt_policy_match(cfg, POLICY_ACTION_ENCRYPT_L3, CRYPTO_MODE_CTR, 128, wire_pol))
        return 0;
...
const struct crypto_option_ops *crypto_opt_l3_ctr128_udp_ops(void)
{
