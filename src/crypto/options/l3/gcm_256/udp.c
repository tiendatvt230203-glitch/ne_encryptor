    ...
    uint8_t *out_buf, size_t out_max, uint32_t *out_len, uint8_t fake_proto)
{
    ...
    ip_payload_len = (size_t)(L3_FRAG_TUNNEL_HDR_SIZE + L3_FRAG_TAG_SIZE + enc_plain_len + AES_GCM_TAG_SIZE);
    ...
    uint16_t pkt_id, size_t out_max, uint32_t *out_len)
{
    ...
    l3_patch_ipv4_fast(ip, (uint16_t)(ip_hdr_len + L3_FRAG_TUNNEL_HDR_SIZE + L3_FRAG_TAG_SIZE +
                                 ...
                                 uint32_t pkt_len, uint16_t *pkt_id, uint8_t *frag_index)
{
    ...
    if (!opt_policy_match(cfg, POLICY_ACTION_ENCRYPT_L3, CRYPTO_MODE_GCM, 256, wire_pol))
        return 0;
...
const struct crypto_option_ops *crypto_opt_l3_gcm256_udp_ops(void)
{
