...
int crypto_opt_l2_gcm128_need_split(uint32_t pkt_len)
{
    return (pkt_len + OPT_FRAG_META_LEN) > crypto_option_get_mtu();
}

int crypto_opt_l2_gcm128_split(struct packet_crypto_ctx *ctx, uint8_t *pkt_data, uint32_t pkt_len,
                           size_t frag0_max, uint32_t *frag0_len,
                           uint8_t *frag1, size_t frag1_max, uint32_t *frag1_len)
{
...
int crypto_opt_l2_gcm128_encrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int l3_off;
    int et_off;
    int n;
...
int crypto_opt_l2_gcm128_decrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int n;
...
int crypto_opt_l2_gcm128_is_fragment(const struct app_config *cfg, const uint8_t *pkt_data,
                                 uint32_t pkt_len, uint16_t *pkt_id, uint8_t *frag_index)
{
    ...
    if (crypto_eth_l2_read_policy_id(pkt_data, pkt_len, &wire_pol) != 0)
        return 0;
    if (!opt_policy_match(cfg, POLICY_ACTION_ENCRYPT_L2, CRYPTO_MODE_GCM, 128, wire_pol))
        return 0;
    opt_read_frag_tag(pkt_data + tag_off + 1, pkt_id, frag_index);
    return (*frag_index <= 1) ? 1 : 0;
}

int crypto_opt_l2_gcm128_reasm(int profile_slot, int worker_idx, struct packet_crypto_ctx *ctx,
...
void crypto_opt_l2_gcm128_frag_gc(int profile_slot, int worker_idx, uint64_t now_ns)
{
