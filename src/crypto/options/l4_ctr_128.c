    ...
    uint8_t *out_buf, size_t out_max, uint32_t *out_len)
{
    ...
    int nonce_len;
    const uint8_t *key;
    int total_overhead = L4_TUNNEL_HDR_SIZE + L4_FRAG_TAG_SIZE;
    ...
    l4_write_tunnel_header_frag(out_buf + tunnel_off, nonce, PACKET_CRYPTO_NONCE_BYTES, ctx->wire_id);
    ...
    uint16_t pkt_id, size_t out_max, uint32_t *out_len)
{
    ...
    if (!key)
        return -1;
    l4_write_tunnel_header_frag(packet + tunnel_off, nonce, PACKET_CRYPTO_NONCE_BYTES, ctx->wire_id);
...
int crypto_opt_l4_ctr128_need_split(uint32_t pkt_len)
{
    return (pkt_len + OPT_FRAG_META_LEN) > crypto_option_get_mtu();
}

int crypto_opt_l4_ctr128_split(struct packet_crypto_ctx *ctx, uint8_t *pkt_data, uint32_t pkt_len,
                           size_t frag0_max, uint32_t *frag0_len,
                           uint8_t *frag1, size_t frag1_max, uint32_t *frag1_len)
{
...
int crypto_opt_l4_ctr128_encrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int l3_off;
    uint8_t ip_proto;
    int ip_hdr_len;
...
int crypto_opt_l4_ctr128_decrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int l3_off;
    uint8_t ip_proto;
    int ip_hdr_len;
...
int crypto_opt_l4_ctr128_is_fragment(const struct app_config *cfg, const uint8_t *pkt_data,
                                 uint32_t pkt_len, uint16_t *pkt_id, uint8_t *frag_index)
{
    ...
    if (!opt_policy_match(cfg, POLICY_ACTION_ENCRYPT_L4, CRYPTO_MODE_CTR, 128, wire_pol))
        return 0;
...
int crypto_opt_l4_ctr128_reasm(int profile_slot, int worker_idx, struct packet_crypto_ctx *ctx,
...
void crypto_opt_l4_ctr128_frag_gc(int profile_slot, int worker_idx, uint64_t now_ns)
{
