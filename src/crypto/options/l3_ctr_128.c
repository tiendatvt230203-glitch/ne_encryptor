    ...
    uint8_t *out_buf, size_t out_max, uint32_t *out_len, uint8_t fake_proto)
{
    ...
    if (crypto_aes_ctr_with_key(key, iv, out_buf + enc_off, (int)enc_plain_len, OPT_AES_BITS) != 0)
        return -1;
    ip_payload_len = (size_t)(L3_TUNNEL_HDR_SIZE + L3_FRAG_TAG_SIZE + enc_plain_len);
    ...
    uint16_t pkt_id, size_t out_max, uint32_t *out_len)
{
    ...
    if (crypto_aes_ctr_with_key(key, iv, packet + enc_off, (int)frag0_plain_len, OPT_AES_BITS) != 0)
        return -1;
    l3_patch_ipv4_fast(ip, (uint16_t)(ip_hdr_len + L3_TUNNEL_HDR_SIZE + L3_FRAG_TAG_SIZE + frag0_plain_len), fake_proto);
    *out_len = (uint32_t)need;
    return 0;
}
...
int crypto_opt_l3_ctr128_need_split(uint32_t pkt_len)
{
    return (pkt_len + OPT_FRAG_META_LEN) > crypto_option_get_mtu();
}

int crypto_opt_l3_ctr128_split(struct packet_crypto_ctx *ctx, uint8_t *pkt_data, uint32_t pkt_len,
                           size_t frag0_max, uint32_t *frag0_len,
                           uint8_t *frag1, size_t frag1_max, uint32_t *frag1_len)
{
...
int crypto_opt_l3_ctr128_encrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int ip_hdr_len;
    int tunnel_off;
    int n;
...
int crypto_opt_l3_ctr128_decrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int ip_hdr_len;
    int tunnel_off;
    int n;
...
int crypto_opt_l3_ctr128_is_fragment(const struct app_config *cfg, const uint8_t *pkt_data,
...
int crypto_opt_l3_ctr128_reasm(int profile_slot, int worker_idx, struct packet_crypto_ctx *ctx,
...
void crypto_opt_l3_ctr128_frag_gc(int profile_slot, int worker_idx, uint64_t now_ns)
{
    struct opt_table *ft = opt_table(profile_slot, worker_idx);
    if (ft)
