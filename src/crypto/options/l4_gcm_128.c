...
int crypto_opt_l4_gcm128_need_split(uint32_t pkt_len)
{
    return (pkt_len + OPT_FRAG_META_LEN) > crypto_option_get_mtu();
}

int crypto_opt_l4_gcm128_split(struct packet_crypto_ctx *ctx, uint8_t *pkt_data, uint32_t pkt_len,
                           size_t frag0_max, uint32_t *frag0_len,
                           uint8_t *frag1, size_t frag1_max, uint32_t *frag1_len)
{
...
int crypto_opt_l4_gcm128_encrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int l3_off;
    uint8_t ip_proto;
    int ip_hdr_len;
...
int crypto_opt_l4_gcm128_decrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int l3_off;
    uint8_t ip_proto;
    int ip_hdr_len;
...
int crypto_opt_l4_gcm128_is_fragment(const struct app_config *cfg, const uint8_t *pkt_data,
                                 uint32_t pkt_len, uint16_t *pkt_id, uint8_t *frag_index)
{
    ...
    if (!opt_policy_match(cfg, POLICY_ACTION_ENCRYPT_L4, CRYPTO_MODE_GCM, 128, wire_pol))
        return 0;
...
int crypto_opt_l4_gcm128_reasm(int profile_slot, int worker_idx, struct packet_crypto_ctx *ctx,
...
void crypto_opt_l4_gcm128_frag_gc(int profile_slot, int worker_idx, uint64_t now_ns)
{
    struct opt_table *ft = opt_table(profile_slot, worker_idx);
    if (ft)
