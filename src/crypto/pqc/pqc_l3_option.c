...
#define L3_FRAG_TUNNEL_HDR_SIZE (PACKET_CRYPTO_NONCE_BYTES + 3)
                                        ...
                                        uint8_t policy_id)
{
    ...
    buf[nonce_size + 2] = L3_FRAG_MAGIC;
}
                         ...
                         int tunnel_off, uint8_t *ip, uint16_t old_totlen, uint8_t fake_proto)
{
    ...
    l3_write_tunnel_header(packet + tunnel_off, nonce, CRYPTO_PQC_NONCE_BYTES, ctx->wire_id);
                         ...
                         int tunnel_off, uint8_t *ip, uint16_t old_totlen)
{
    ...
    if (crypto_pqc_sess_load(ctx, &pqc) != 0)
        return -1;
    l3_read_tunnel_header(packet + tunnel_off, CRYPTO_PQC_NONCE_BYTES, nonce, NULL, NULL);
    if (crypto_pqc_decrypt_payload(&pqc, nonce, packet + enc_off,
                                   (int)total_after, &dec_len) != 0)
        return -1;
    if (dec_len < L3_ORIG_PROTO_LEN)
        return -1;
    orig_proto = packet[enc_off + dec_len - 1];
    plain_len = (size_t)dec_len - (size_t)L3_ORIG_PROTO_LEN;
    ...
    uint8_t *out_buf, size_t out_max, uint32_t *out_len, uint8_t fake_proto)
{
    ...
    int enc_off = tunnel_off + L3_FRAG_TUNNEL_HDR_SIZE + L3_FRAG_TAG_SIZE;
    int new_len = 0;
    ...
    l3_write_tunnel_header_frag(out_buf + tunnel_off, nonce, CRYPTO_PQC_NONCE_BYTES, ctx->wire_id);
    opt_write_frag_tag(out_buf + tunnel_off + L3_FRAG_TUNNEL_HDR_SIZE, pkt_id, frag_index);
    if (crypto_pqc_encrypt_payload(&pqc, nonce, out_buf + enc_off, (int)enc_plain_len, &new_len) != 0)
        return -1;
    ip_payload_len = (size_t)(L3_FRAG_TUNNEL_HDR_SIZE + L3_FRAG_TAG_SIZE + new_len);
    ...
    uint16_t pkt_id, size_t out_max, uint32_t *out_len)
{
    ...
    if (crypto_pqc_generate_nonce(nonce) != 0)
        return -1;
    l3_write_tunnel_header_frag(packet + tunnel_off, nonce, CRYPTO_PQC_NONCE_BYTES, ctx->wire_id);
    opt_write_frag_tag(packet + tunnel_off + L3_FRAG_TUNNEL_HDR_SIZE, pkt_id, 0);
    if (crypto_pqc_encrypt_payload(&pqc, nonce, packet + enc_off, (int)frag0_plain_len, &new_len) != 0)
        return -1;
    l3_patch_ipv4_fast(ip, (uint16_t)(ip_hdr_len + L3_FRAG_TUNNEL_HDR_SIZE + L3_FRAG_TAG_SIZE + new_len), fake_proto);
    *out_len = (uint32_t)(enc_off + new_len);
    return 0;
}
                               ...
                               uint16_t *out_pkt_id, uint8_t *out_frag_index)
{
    ...
    enc_off = tunnel_off + L3_FRAG_TUNNEL_HDR_SIZE + L3_FRAG_TAG_SIZE;
    if (pkt_len < (size_t)enc_off)
        return -1;
    if (!l3_is_frag_tunnel(packet + tunnel_off, PACKET_CRYPTO_NONCE_BYTES))
        return -1;
    opt_read_frag_tag(packet + tunnel_off + L3_FRAG_TUNNEL_HDR_SIZE, out_pkt_id, out_frag_index);
...
static int l3_udp_encrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int ip_hdr_len;
    int tunnel_off;
    int n;
...
static int l3_udp_decrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int ip_hdr_len;
    int tunnel_off;
    int n;
...
static int l3_udp_need_split(uint32_t pkt_len)
{
    return (pkt_len + OPT_FRAG_META_LEN) > crypto_option_get_mtu();
}

static int l3_udp_split(struct packet_crypto_ctx *ctx, uint8_t *pkt_data, uint32_t pkt_len,
                           size_t frag0_max, uint32_t *frag0_len,
                           uint8_t *frag1, size_t frag1_max, uint32_t *frag1_len)
{
...
static int l3_udp_is_fragment(const struct app_config *cfg, const uint8_t *pkt_data,
                                 uint32_t pkt_len, uint16_t *pkt_id, uint8_t *frag_index)
{
    ...
    if (pkt_len < (uint32_t)(tunnel_off + L3_FRAG_TUNNEL_HDR_SIZE + L3_FRAG_TAG_SIZE))
        return 0;
    if (tunnel_off + PACKET_CRYPTO_NONCE_BYTES + 2 >= (int)pkt_len)
        return 0;
    if (pkt_data[tunnel_off + PACKET_CRYPTO_NONCE_BYTES + 2] != L3_FRAG_MAGIC)
        return 0;
    wire_pol = pkt_data[tunnel_off + PACKET_CRYPTO_NONCE_BYTES + 1];
    if (!opt_policy_match(cfg, POLICY_ACTION_ENCRYPT_L3, CRYPTO_MODE_PQC, 256, wire_pol))
        return 0;
    opt_read_frag_tag(pkt_data + tunnel_off + L3_FRAG_TUNNEL_HDR_SIZE, pkt_id, frag_index);
    return (*frag_index <= 1) ? 1 : 0;
}

static int l3_udp_reasm(int profile_slot, int worker_idx, struct packet_crypto_ctx *ctx,
                           ...
                           uint8_t *out_buf, uint32_t *out_len)
{
    ...
    *pkt_len = (uint32_t)nd;
    ...
    if (!ft)
        return -1;
    rr = l3_reassemble(ft, pkt_data, *pkt_len,
...
static void l3_udp_frag_gc(int profile_slot, int worker_idx, uint64_t now_ns)
{
    struct opt_table *ft = opt_table(profile_slot, worker_idx);
    if (ft)
        opt_frag_gc_table(ft, now_ns);
}
static int l3_tcp_encrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int ip_hdr_len;
    int tunnel_off;
    int n;
...
static int l3_tcp_decrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int ip_hdr_len;
    int tunnel_off;
    int n;
...
static int l3_icmp_encrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int ip_hdr_len;
    int tunnel_off;
    int n;
...
static int l3_icmp_decrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int ip_hdr_len;
    int tunnel_off;
    int n;
...
static int l3_ospf_encrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int ip_hdr_len;
    int tunnel_off;
    int n;
...
static int l3_ospf_decrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int ip_hdr_len;
    int tunnel_off;
    int n;
...
CRYPTO_OPS_PLAIN(crypto_opt_l3_pqc_tcp_ops, l3_tcp_encrypt, l3_tcp_decrypt)
CRYPTO_OPS_PLAIN(crypto_opt_l3_pqc_icmp_ops, l3_icmp_encrypt, l3_icmp_decrypt)
CRYPTO_OPS_PLAIN(crypto_opt_l3_pqc_ospf_ops, l3_ospf_encrypt, l3_ospf_decrypt)

const struct crypto_option_ops *crypto_opt_l3_pqc_udp_ops(void)
{
    static const struct crypto_option_ops ops = {
        .need_split = l3_udp_need_split,
        .split = l3_udp_split,
        .encrypt = l3_udp_encrypt,
        .decrypt = l3_udp_decrypt,
        .is_fragment = l3_udp_is_fragment,
        .reasm = l3_udp_reasm,
        .frag_gc = l3_udp_frag_gc,
    };
    return &ops;
}
