...
{
#if MODE_L2_PQC
    uint8_t frag2[NE_PKT_MAX];
    uint32_t frag2_len = 0;
    if (l2_encrypt_maybe_fragment(pkt, &len, NE_PKT_MAX, frag2, &frag2_len) != 0)
        return;
    if (frag2_len > 0)
        (void)mtu9k_tx_pkt(p, &p->wan, frag2, frag2_len);
#endif
...
static void process_wan_to_lan(struct mtu9k_pair *p, uint8_t *pkt, uint32_t len)
{
    ...
    if (l2_has_enc_marker(pkt, len)) {
        int st;
        st = l2_decrypt_maybe_reassemble(pkt, &len);
        if (st < 0)
            return;
        if (st > 0)
            return;
