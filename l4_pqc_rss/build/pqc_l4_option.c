...
#include "shim/l4pqc_iface_shim.h"
#include "shim/cpu_map.h"
...
static int l4_udp_encrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int l3_off;
    uint8_t ip_proto;
    int ip_hdr_len;
...
static int l4_udp_decrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int l3_off;
    uint8_t ip_proto;
    int ip_hdr_len;
...
static int l4_udp_need_split(uint32_t pkt_len)
{
    return (pkt_len + OPT_FRAG_META_LEN) > crypto_option_get_mtu();
}

static int l4_udp_split(struct packet_crypto_ctx *ctx, uint8_t *pkt_data, uint32_t pkt_len,
                           size_t frag0_max, uint32_t *frag0_len,
                           uint8_t *frag1, size_t frag1_max, uint32_t *frag1_len)
{
...
static int l4_udp_is_fragment(const struct app_config *cfg, const uint8_t *pkt_data,
...
static int l4_udp_reasm(int profile_slot, int worker_idx, struct packet_crypto_ctx *ctx,
...
static void l4_udp_frag_gc(int profile_slot, int worker_idx, uint64_t now_ns)
{
    struct opt_table *ft = opt_table(profile_slot, worker_idx);
    if (ft)
        opt_frag_gc_table(ft, now_ns);
}
static int l4_tcp_encrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int l3_off;
    uint8_t ip_proto;
    int ip_hdr_len;
...
static int l4_tcp_decrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt, uint32_t *pkt_len)
{
    int l3_off;
    uint8_t ip_proto;
    int ip_hdr_len;
...
const struct crypto_option_ops *crypto_opt_l4_pqc_udp_ops(void)
{
    static const struct crypto_option_ops ops = {
        .need_split = l4_udp_need_split,
        .split = l4_udp_split,
        .encrypt = l4_udp_encrypt,
        .decrypt = l4_udp_decrypt,
        .is_fragment = l4_udp_is_fragment,
        .reasm = l4_udp_reasm,
        .frag_gc = l4_udp_frag_gc,
    };
    return &ops;
}